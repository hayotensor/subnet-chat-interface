...
function isWaitingForInputs() {
  ...
  console.log('isWaitingForInputs inputs', $('.human-replica textarea'))
  console.log('isWaitingForInputs inputs length', $('.human-replica textarea').length)
  console.log('isWaitingForInputs length >= 1', $('.human-replica textarea').length >= 1)
...
function sendReplica() {
  ...
  if (isWaitingForInputs()) {
    ...
    console.log("sendReplica human replkica value", $('.human-replica:last textarea').val())
...
function appendTextArea() {
  ...
  $('#human-input-text').val(humanPrompt)
  upgradeTextArea();
}

function upgradeTextArea() {
  const textareaO = $('.human-replica textarea');
  autosize(textareaO);
  textareaO[0].selectionStart = textareaO[0].value.length;
  textareaO.focus();
  ...
  textarea.on('keypress', e => {
    if (e.which == 13 && !e.shiftKey) {
      ...
      console.log("human-input-text value", value)
      // $('.dialogue').append($(
      //   `<p class="human-replica">Human: ${value}</p>`
      // ))
      $('.human-replica textarea').val(value);
